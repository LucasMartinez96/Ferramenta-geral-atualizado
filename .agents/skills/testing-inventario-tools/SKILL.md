...
```
- SheetJS is now unified to **`https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js`**
  across the CDN-based tools (a previous cdnjs `0.20.2` URL used to 404 and broke the
  `.xlsx` path — if you see `SheetJS não carregou.` again, check the CDN URL/version).
  `CICLICO ALM51.html` embeds SheetJS inline/offline on purpose (no CDN).
- Sample spreadsheets live in the repo root: `inventario geral1.xlsx` (~10.8k rows),
  `ALM511.xlsx`, `Base perecivel.xlsx` — handy for exercising the real `.xlsx` parse path.
- File upload uses the native GTK "Open File" dialog. Easiest: press **Ctrl+L** in the
  dialog and type the absolute path (e.g. `/home/ubuntu/repos/<repo>/inventario geral1.xlsx`),
  then Enter. Files in `~` (Home) also show at the dialog root for double-click.
...
## Testing alert()->Toast changes
Tools now load a shared classic script `js/toast.js` that exposes a global
`window.showToast(message, type?, duration?)`; all `alert(...)` calls were replaced with
`showToast(...)`. The helper **infers severity from the message text** when `type` is
omitted (order: error > warning > success > info), so the same call site renders a
different color depending on wording:
- success = **green** (e.g. "Arquivo carregado! Linhas: N", contains `carregad`)
- warning = **amber** (e.g. "Preencha todos os campos!", contains `preencha`)
- error = **red** (e.g. "Valores inválidos", contains `inválid`)
Toasts appear top-right, are **non-blocking** (no native dialog), and auto-dismiss.
Adversarial check: a broken impl either shows the old blocking `alert()` modal, throws
(no toast + action never completes), or renders the wrong color. Easy triggers:
`PORTA-PORTA.html` → click **Adicionar Registro** with empty fields (amber) / with
non-numeric Qtd (red); any CICLICO/Indicador tool → upload a CSV (green).
